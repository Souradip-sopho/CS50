...
char* indexes(const char* path)
{
    ...
    {
        newpath = malloc(strlen(path) + strlen(indexhtml) + 2);
        strcpy(newpath, path);
        newpath[strlen(path)]='/';
        strcat(newpath, indexhtml);
        newpath[strlen(path) + strlen(indexhtml)+1] = '\0';
    }
    else
    {
    ...
    if(path[strlen(path)-1]!='/')
        {
        newpath = malloc(strlen(path) + strlen(indexphp) + 2);
        strcpy(newpath, path);
        newpath[strlen(path)]='/';
        strcat(newpath, indexphp);
        newpath[strlen(path) + strlen(indexphp)+1] = '\0';
        }
    else
        {
...
{
    // TODO
        char* fgap=strchr(line,' ');
        if(*(fgap+1)==' ')
        {error(405);return false;}
        char* sgap=strrchr(line,' ');
        if(*(sgap-1)==' ')
